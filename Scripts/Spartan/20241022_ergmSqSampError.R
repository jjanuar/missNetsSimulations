...
# what do I need?
depletedNetworkInd = as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))
entrValue = as.numeric(Sys.getenv("entrValue"))
...
missCoefs = c(0, 0.4, 0.5, 0.4)  # everything set to positive is dangerous for mcmc explosions, but fixed density helps.
...
load(here("Data", "20241021_ergmSqDepNets.RData"))

## Initialising the network
peterAdjmat = as.network(depletedNetList[[depletedNetworkInd]], directed = FALSE)
...
# try it out
outputNoEntrOut = ergmSquaredSampler(formula = peterModel, 
                            adjMat = peterAdjmat,
...
# and save the object
save(outputNoEntrOut, 
     file = here("Output", "20241022_ergmSqSampError", 
                 paste(format(Sys.Date(), "%Y%m%d"), "_ergmSqOutDepNet",depletedNetworkInd, "_entrVal", entrValue,".RData", sep = "")))