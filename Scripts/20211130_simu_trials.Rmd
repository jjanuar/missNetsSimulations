...
Let $D_{ij}$ represent a missing data indicator matrix for adjacency matrix $X$.

$$D_{ij} = \begin{cases} 0 & \text{if } { X_{ij} = 1}\\ 1 & \text{if }{X_{ij}} = \text{missing}\\ \end{cases}$$

$D_{ij} = 1$ only when the edge $X_{ij}$ is missing.

This format of data is very similar to an adjacency matrix $X_{ij}$

$$X_{ij} = \begin{cases} 0 & \text{if } i = j\\ 1 & \text{if } i \text{ sends a tie to } j\\ \end{cases}$$

Also note, $X_{ij}: i, j \in \{1, ..., n\}$ for $n$ participants.
...
$$P(D_{ij}|X_{ij}) = \prod_{ij} \frac{\exp(\alpha + \beta x_{ij} d_{ij})}{1 + \exp(\alpha + \beta x_{ij}d_{ij})}$$

Which if you plug into a logit function,

$$logit(P_{ij}) = \alpha + \beta x_{ij} d_{ij}$$

This is a case with a simple generalised linear model (logistic reg.), extending it to the ERGM would be like

$$P(D_{ij}| X_{ij}, \alpha, \beta) = \frac{exp(\alpha \sum d_{ij} + \beta \sum x_{ij}d_{ij})}{\phi(\alpha, \beta)}$$
...
Note that the $\theta$ here is the same $\theta$ being used to estimate the observed ties $P(X_{obs} | X_{miss}, \theta)$

So an 'advantage' (?) would be that modelling $D_{ij}$ could use a different set of parameters than the Bayesian model.

That is, $\theta$ in $P(X_{miss}|X_{obs}, \theta)$ is the same as $P(X_{obs}|X_{miss}, \theta)$ whereas $P(D_{ij}|X_{ij}, \theta)$ can be different to $P(X_{ij}|D_{ij}, \theta)$.
...
Not entirely sure. If we know how the missing ties are generated, then we might apply that model to predict what their potential observable values were. e.g., $X_{miss} | X_{obs}, \theta$
